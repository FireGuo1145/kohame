...
export function Avatar({name,size="sm"}:{name:string;size?:"sm"|"lg"}){return <span className={`${size==="lg"?"size-16 text-2xl":"size-7 text-xs"} grid shrink-0 place-items-center rounded-full bg-zinc-900 font-semibold text-white dark:bg-zinc-100 dark:text-zinc-900`}>{name.slice(0,1).toUpperCase()}</span>}
