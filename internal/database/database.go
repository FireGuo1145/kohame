...
func Open(cfg config.DatabaseConfig) (*gorm.DB, error) {
	...
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS milestones (id ` + idColumn + `, repository_name VARCHAR(161) NOT NULL, title VARCHAR(160) NOT NULL, description TEXT NOT NULL DEFAULT '', due_at TIMESTAMP, state VARCHAR(16) NOT NULL DEFAULT 'open', created_at TIMESTAMP NOT NULL, UNIQUE(repository_name, title))`,
		`CREATE TABLE IF NOT EXISTS issue_assignees (issue_id BIGINT NOT NULL, user_id BIGINT NOT NULL, PRIMARY KEY(issue_id, user_id))`,
		...
		`CREATE TABLE IF NOT EXISTS webhooks (id ` + idColumn + `, repository_name VARCHAR(161) NOT NULL, url VARCHAR(2048) NOT NULL, secret TEXT NOT NULL DEFAULT '', events TEXT NOT NULL DEFAULT 'push', active BOOLEAN NOT NULL DEFAULT TRUE, created_at TIMESTAMP NOT NULL, updated_at TIMESTAMP NOT NULL)`,
